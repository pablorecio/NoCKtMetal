...
public:
	...
	/**
	 * Constructor de un objeto de la clase <code>Inventario</code>.
	 *
	 * @param rXML Ruta donde se almacenará el fichero serializado de este objeto.
	 */
	Inventario(string rXML) :
		_ruta_XML(rXML) {
	}

	/**
	 * Construye un objeto de la clase <code>Inventario</code> a partir de
	 * un fichero XML.
	 *
	 ...
	 */
	const std::map<Uint32, Objeto>& getInventarioCompleto() const {
		return _inventario;
	}
	 ...
	 */
	std::map<Uint32, Objeto>& getInventarioCompleto() {
		return _inventario;
	}
