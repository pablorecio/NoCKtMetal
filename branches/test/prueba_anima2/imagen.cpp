...
void Imagen::dibujarFondo(Sint32 cx, Sint32 cy, SDL_Surface *pantalla){
  ...
  Uint32 aux_h = _cY+(pantalla->h/_Tam_Tile);
  Uint32 aux_w = _cX+(pantalla->w/_Tam_Tile);
  ...
  for(Uint32 i=_cX; i<aux_w && i<_ancho; i++){
	...
	cout << "(i,j): ("<< i << "," << j <<")." << endl;
        ...
        origen.h = _tiles[_matrizOriginal[i][j]]->getAltura();
	origen.w = _tiles[_matrizOriginal[i][j]]->getAnchura();
	
	destino.x=(i-_cX)*(_tiles[_matrizOriginal[i][j]]->getAnchura());
	destino.y=(j-_cY)*(_tiles[_matrizOriginal[i][j]]->getAltura());
...
Sint32 Imagen::getCoorX(){ return _cX;}
Sint32 Imagen::getCoorY(){ return _cY;}
